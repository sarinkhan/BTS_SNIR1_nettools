...
def main():
    ...
    file = open("host_list.txt", "r")
    host_list = file.readlines()  # on récupère chaque ligne
    # les lignes sont stockées dans une liste
    for host in host_list:  # on parcourt la liste
        print("host " + host[:-1])  # host[:-1] enlève le dernier caractère
